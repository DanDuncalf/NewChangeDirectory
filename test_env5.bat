...
echo === Start-Process with -Wait ===
powershell -NoProfile -Command "$env:LOCALAPPDATA='%LOCALAPPDATA%'; $env:NCD_TEST_MODE='1'; $env:NCD_UI_KEYS='ENTER'; Start-Process -NoNewWindow -Wait -FilePath '%NCD%' -ArgumentList '%CONF_OVERRIDE% /d "%DB_OVERRIDE%" nonexistent_xyz_42'; exit $LASTEXITCODE"
