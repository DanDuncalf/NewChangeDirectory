...
echo === Start-Process search (WITH output capture) ===
powershell -NoProfile -Command "$env:LOCALAPPDATA='%LOCALAPPDATA%'; $env:NCD_TEST_MODE='1'; $env:NCD_UI_KEYS='ENTER'; $p = Start-Process -PassThru -NoNewWindow -FilePath '%NCD%' -ArgumentList '%CONF_OVERRIDE% /d "%DB_OVERRIDE%" nonexistent_xyz_42'; if (-not $p.WaitForExit(10000)) { $p.Kill(); exit 99 } else { exit $p.ExitCode }"
echo Batch ERRORLEVEL: %ERRORLEVEL%
