...
echo Batch LOCALAPPDATA: %LOCALAPPDATA%

echo === Direct search ===
"%NCD%" %CONF_OVERRIDE% /d "%DB_OVERRIDE%" /agent check --stats

echo === Start-Process search ===
powershell -NoProfile -Command "$env:LOCALAPPDATA='%LOCALAPPDATA%'; $env:NCD_TEST_MODE='1'; $env:NCD_UI_KEYS='ENTER'; $p = Start-Process -PassThru -NoNewWindow -Wait -FilePath 'cmd' -ArgumentList '/c set LOCALAPPDATA'"

echo === Start-Process NCD stats ===
powershell -NoProfile -Command "$env:LOCALAPPDATA='%LOCALAPPDATA%'; $env:NCD_TEST_MODE='1'; $env:NCD_UI_KEYS='ENTER'; $p = Start-Process -PassThru -NoNewWindow -Wait -FilePath '%NCD%' -ArgumentList '%CONF_OVERRIDE% /d "%DB_OVERRIDE%" /agent check --stats'"

echo === Start-Process NCD search ===
powershell -NoProfile -Command "$env:LOCALAPPDATA='%LOCALAPPDATA%'; $env:NCD_TEST_MODE='1'; $env:NCD_UI_KEYS='ENTER'; $p = Start-Process -PassThru -NoNewWindow -Wait -FilePath '%NCD%' -ArgumentList '%CONF_OVERRIDE% /d "%DB_OVERRIDE%" nonexistent_xyz_42'"
